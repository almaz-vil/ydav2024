...
class ApplicationYdav: Application() {
    ...
    override fun onCreate() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            ...
            registerReceiver(
                smsOutputSentBroadcastReceiver,
                IntentFilter("ru.dimon.ydav2024.SENT_SMS_ACTION"),
                RECEIVER_NOT_EXPORTED
            )
            val smsOutputDeliveryBroadcastReceiver = SmsInputBroadcastReceiver()
            registerReceiver(
                smsOutputDeliveryBroadcastReceiver,
                IntentFilter("ru.dimon.ydav2024.DELIVERY_SMS_ACTION"),
                RECEIVER_NOT_EXPORTED
            )
        }else{
            val smsOutputSentBroadcastReceiver = SmsOutputSentBroadcastReceiver()
